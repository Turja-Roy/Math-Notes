...
\Theorem{Convergence of Power Series}{
    ...
    \begin{itemize}
        \item \textbf{Ratio Test:} Compute $\displaystyle L = \lim_{n \to \infty} \left| \frac{a_{n+1}}{a_n} \right|$ where $a_n = c_n(x-a)^n$. The series converges when $L < 1$, which gives $|x-a| < R$.
        \item \textbf{Root Test:} Compute $\displaystyle L = \lim_{n \to \infty} \sqrt[n]{|a_n|}$ where $a_n = c_n(x-a)^n$. The series converges when $L < 1$, which gives $|x-a| < R$.
    \end{itemize}

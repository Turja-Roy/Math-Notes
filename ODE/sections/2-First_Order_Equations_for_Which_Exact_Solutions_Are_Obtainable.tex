...
\begin{equation} \label{eq:derivative-form}
    \dv{y}{x} = f(x,y)
\end{equation}
...
The equation \[
    ...
    \]
    is the form \eqref{eq:derivative-form}. It may be written as \[
        (x^2 + y^2) \dd{x} + (y-x) \dd{y} = 0
    \] which is of the form \eqref{eq:differential-form}.
    
    Again, the equation \[
        (\sin x + y) \dd{x} + (x + 3y) \dd{y} = 0
    \] is of the form \eqref{eq:differential-form}, which can also be written as \[
...
Let $F$ be a function of two real variables such that  $F$ has continuous first partial derivatives in a domain $D$. The total differential $dF$ of the function $F$ is defined by the formula
    \[
        d{F(x,y)} = \pdv{F(x,y)}{x} \dd{x} + \pdv{F(x,y)}{y} \dd{y}
    \]
    for all $(x,y) \in D$.
} \vspace{0.25cm}

Comparing $dF(x,y)$ with the form \eqref{eq:differential-form}, we get
\[
    \pdv{F(x,y)}{x} = M(x,y) \quad \text{and} \quad
    \pdv{F(x,y)}{y} = N(x,y)
\]
...
The expression
    \begin{equation} \label{eq:exact-diff-expression}
        M(x,y) d{x} + N(x,y) d{y}
    \end{equation}
    ...
    That is, expression \eqref{eq:exact-diff-expression} is an exact differential in $D$ if there exists a function $F$ such that
\[
    \pdv{F(x,y)}{x} = M(x,y) \quad \text{and} \quad
    \pdv{F(x,y)}{y} = N(x,y)
\]
...
\Theorem{Exact Differential Equation}{
\begin{enumerate}
        \item If the DE $M(x,y) d{x} + N(x,y) d{y} = 0$ is exact in $D$, then
            \[ \forall (x,y)\in D, \quad \pdv{M(x,y)}{y} = \pdv{N(x,y)}{x} \]
        \item Conversely, if 
            \[ \forall (x,y)\in D, \quad \pdv{M(x,y)}{y} = \pdv{N(x,y)}{x} \]
...
\begin{align*}
    \pdv{F(x,y)}{x} = M(x,y) \quad&, \quad \pdv{F(x,y)}{y} = N(x,y)  \\
    \pdv[2]{F(x,y)}{y}{x} = \pdv{M(x,y)}{y} \quad&, \quad \pdv[2]{F(x,y)}{y}{x} = \pdv{N(x,y)}{x}
\end{align*}
\begin{align*}
    &\because \pdv[2]{F(x,y)}{x}{y} = \pdv[2]{F(x,y)}{y}{x} \\
...
\Theorem{Solution of Exact DE}{
\\If $M(x,y) d{x} + N(x,y) d{y} = 0$ is exact in domain $D$, then
    \[
        \forall(x,y)\in D, \exists F(x,y) : \pdv{F(x,y)}{x} = M(x,y) \quad \text{and} \quad
        \pdv{F(x,y)}{y} = N(x,y)
    \]
...
\Example{
    ...
    \[
        (2x\cos y + 3x^2y) d{x} + (x^3 - x^2\sin y - y) d{y} = 0, \quad y(0) = 2
    \] \vspace{-40pt}
}{
\begin{align*}
        (2x\cos y \dd{x} - x^2\sin y d{y}) + (3x^2y d{x} + x^3 d{y}) - y d{y} &= 0 \\
...
\Definition{Integrating Factor (IF)}{
If the DE
    \begin{equation} \label{eq:non-exact-de}
        M(x,y) d{x} + N(x,y) d{y} = 0
    \end{equation}
    is not exact in a domain $D$ but the DE
    \begin{equation} \label{eq:exact-after-if}
...
Consider the DE
    ...
    \]
    This equation is of the form \eqref{eq:non-exact-de}, where
     \begin{align*}
        M(x,y) &= 3y+4xy^2, \quad & N(x,y) &= 2x+3x^2y \\
        \pdv{M(x,y)}{y} &= 3+8xy, \quad & \pdv{N(x,y)}{x} &= 2+6xy
    \end{align*}
    Since \[
        \pdv{M(x,y)}{y} \neq \pdv{N(x,y)}{x}
    \] except for $(x,y)$ such that $2xy + 1 = 0$, this equation is not exact in any rectangular domain $D$.
    
    Let $\mu(x,y) = x^2y$. Then the corresponding DE of the form \eqref{eq:exact-after-if} is \[
    ...
    \]
    For all real $(x,y)$. Hence, $\mu(x,y) = x^2y$ is an integrating factor of this equation.
}
...
\Example{
    Determine the constant $A$ in the following equations such that the equation is exact
    \begin{enumerate}
        \item $\displaystyle (Ax^2y + 2y^2) d{x} + x^3 + 4xy d{y} = 0$
        \item $\displaystyle \left( \frac{Ay}{x^3} + \frac{y}{x^2} \right) d{x} + \left( \frac{1}{x^2} - \frac{1}{x} \right) d{y} = 0$
    \end{enumerate}
}{
\textbf{1.}
...
\Example{
    Solve the initial-value problem that consists of the DE
    \[
...
\Definition{Homogeneous Equation of degree n}{
...
\begin{equation}
    \label{eq:homogeneous-separable}
    \frac{d{v}}{v - g(v)} + \frac{d{x}}{x} = 0 \qed
\end{equation}

\Theorem{Solution of a Homogeneous Differential Equation}{
To solve a DE of the form \eqref{eq:homogeneous-form}, we let $y = vx$ and transform the homogeneous equation into a separable equation of the form \eqref{eq:homogeneous-separable}. From this, we have \[
...
\Example{
    Solve the initial-value problem
    \[
        (y + \sqrt{x^2 + y^2}) d{x} - x d{y} = 0, \quad y(1) = 0
    \]
}{
Derivative form: \[
...
\textbf{1.}
    ...
    \[ (2xy \dd{x} + x^2 \dd{y}) + (y^2 \dd{x} + 2xy \dd{y}) + 2x^2 \dd{x} = 0 \]
...
A first-order ordinary differential equation is linear in the dependent variable $y$ and independent variable $x$ if it is, or can be, written in the form
 ...
 Equation \eqref{eq:linear-form} can also be written as
    \begin{equation}
        \label{eq:linear-differential-form}
        [P(x)y - Q(x)] d{x} + d{y} = 0 
    \end{equation}
 Here, \[
     \pdv{M(x,y)}{y} = P(x) \quad\text{ and }\quad \pdv{N(x,y)}{x} = 0
 \]
 The equation is not exact. So we multiply both sides of \eqref{eq:linear-differential-form} by an integrating factor: \[
     [\mu(x)P(x)y - \mu(x)Q(x)] d{x} + \mu(x) d{y} = 0
 \]
 Now,
\begin{align*}
    \pdv{}{y} \left[\mu(x)M(x,y)\right] &= \pdv{}{x} \left[\mu(x)N(x,y)\right] \\
    \pdv{}{y} \left[\mu(x)P(x)y - \mu(x)Q(x)\right] &= \pdv{}{x} \left[\mu(x)\right] \\
    % \mu(x) P(x) &= \dv{\mu(x)}{x} \\
    \mu P(x) &= \frac{d}{dx} \mu \\
...
\Theorem{Solution of Linear Differential Equation}{
...
\vspace{-20pt}
    ...
    \begin{align*}
        y^3\dv{x}{y} + 3xy^2 &= y \\
        y^3 \dd{x} + (3xy^2 - y) \dd{y} &= 0 \\
        (y^3 \dd{x} + 3xy^2 \dd{y}) - y \dd{y} &= 0 \\
...
\Theorem{Transformation of Bernoulli Equation to Linear Equation}{
...
\[ y^{-3}\dv{y}{x} + y^{-2} = x \]
    ...
    \begin{align*}
        e^{-2x}\dv{v}{x} - 2e^{-2x}v &= -2xe^{-2x} \\
        \frac{d}{d{x}} (e^{-2x}v) &= -2xe^{-2x} \\
        e^{-2x}v &= \int{-2xe^{-2x}} d{x} + c_1
    \end{align*}
...
\begin{equation}
    ...
    \mu(x,y)M(x,y) d{x} + \mu(x,y)N(x,y) d{y} = 0
\end{equation}
is exact. Now, we can say the equation \eqref{eq:if-exact} is exact if and only if \[
...
\begin{equation}
    \label{eq:if-pde}
    N(x,y)\pdv{\mu}{x} - M(x,y)\pdv{\mu}{y} = \left[ \pdv{M(x,y)}{y} - \pdv{N(x,y)}{x} \right] \mu
\end{equation}
Equation \eqref{eq:if-pde} is a PDE for the general IF $\mu$, and we're in no position to attempt to solve such an equation. Let's attepts to determine IF of certain special types instead.\\
If $M$ and $N$ are functions of $x$ and $y$, but the IF $\mu$ depends only upon $x$, then equation \eqref{eq:if-pde} reduces to
\[
...
\]
depends upon $x$ only, equation \eqref{eq:if-x-only} is a separable ordinary equation in the single independent, equation \eqref{eq:if-x-only} is a separable ordinary equation in the single independent variable $x$ and the single dependent variable $\mu$. In this case, we may integrate to obtain the IF
\[
...
\\Consider the differential equation
    ...
    If
    \begin{equation}
        \label{eq:if-condition-x}
        \frac{1}{N(x,y)} \left[ \pdv{M(x,y)}{y} - \pdv{N(x,y)}{x} \right]  
    \end{equation}
    depends upon $x$ only, then IF
    \begin{equation}
        \label{eq:if-formula-x}
        \mu(x) = \exp \left\{ \int{ \frac{1}{N(x,y)} \left[ \pdv{M(x,y)}{y} - \pdv{N(x,y)}{x} \right] } d{x} \right\}
    \end{equation}
    And if
    \begin{equation}
...
This equation is not any of the five basic types of differential equations. We can apply Theorem 2.4.1 in this case. Here, $M(x,y) = 2x^2+y$ and $N(x,y) = x^2y-x$, and the expression \eqref{eq:if-condition-x} becomes \[
...
\Theorem{A Special Transformation}{
\\Consider the equation
    \begin{equation}
...
Here, \[
    ...
    \]
    \begin{align*}
        \therefore (z+3) \dd{x} + (2z-1) \left( \frac{dz-dx}{2} \right) &= 0 \\
        7 \dd{x} + (2z-1) dz &= 0 \\
        7x + z^2 - z &= c \\

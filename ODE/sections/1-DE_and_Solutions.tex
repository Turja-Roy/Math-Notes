...
\Example{Ordinary Differential Equations:}{
    \begin{enumerate}
        \item $\displaystyle \dv{y}{x} + x \left( \dv{y}{x} \right)^2 = 0$
        \item $\displaystyle \dv[4]{x}{t} + 5 \dv[2]{x}{t} + 3x = \sin{t}$
    \end{enumerate}
}
...
\Example{Partial Differential Equations:}{
    \begin{enumerate}
        \item $\displaystyle \pdv{v}{s} + \pdv{v}{t} = v$
        \item $\displaystyle \pdv[2]{u}{x} + \pdv[2]{u}{y} + \pdv[2]{u}{z} = 0$
    \end{enumerate}
}
...
\Definition{Linearity of Differential Equations}{
...
\Example{Ordinary Differential Equations: Orders, Degree, Linearity}{
    \begin{align*}
        \dv[3]{y}{x} - 3\dv[2]{y}{x} + 3\dv{y}{x} - 6y = \sin{x} &\text \qquad \text{3rd ord 1st deg Lin} \\
        \dv[2]{y}{x} + \left( \dv{y}{x} \right)^2 + y = 0 &\text \qquad \text{2nd ord 1st deg Non-Lin} \\
        y = x\dv{y}{x} + \sqrt{1 + \dv[2]{y}{x} } &\text \qquad \text{2nd ord 1st deg Non-Lin} \\
        \dv[4]{x}{t} + t^2\dv[3]{x}{t} + \dv{x}{t} = \sin{t} &\text \qquad \text{4th ord 1st deg Lin} \\
        \dv[2]{y}{x} + 5\dv{y}{x} + 6y^2 = 0 &\text \qquad \text{2nd ord 1st deg Non-Lin} \\
        \dv[2]{y}{x} + 5 \left( \dv{y}{x} \right)^3 + 6y = 0 &\text \qquad \text{2nd ord 1st deg Non-Lin} \\
        \dv[2]{y}{x} + 5y\dv{y}{x} + 6y = 0 &\text \qquad \text{2nd ord 1st deg Non-Lin} \\
        \left( \dv[2]{y}{x} \right)^3 + 5\dv{y}{x} + 6y = 0 &\text \qquad \text{2nd ord 3rd deg Non-Lin}
    \end{align*}
}
...
\begin{equation} \label{eq:nth-order-de}
    F \left[ x, y, \dv{y}{x}, \cdots, \dv[n]{y}{x} \right] = 0
\end{equation}

\Definition{Explicit solution}{
    $f$ is an explicit solution of \eqref{eq:nth-order-de} if \[
        \forall x \in I, F \left[ x, f(x), f'(x), \cdots, f^{(n)}(x) \right] = 0
    \] where $I$ is a real interval.
}

\Definition{Implicit solution}{
    $g(x,y)=0$ is an implicit solution if this relation defines at least one real function $f(x)$ on an interval $I$ such that $f$ is an explicit solution of \eqref{eq:nth-order-de}
}

\Example{Explicit and Implicit Solutions}{
    \begin{align*}
        &x + y \dv{y}{x} = 0 &&: \text{Differential Equation} \\
        &x^2+y^2-25 = 0 &&: \text{Implicit solution} \\
        &y = \pm \sqrt{25 - x^2} \quad;\quad -5 \le x \le 5 &&: \text{Explicit solution}
    \end{align*}
}
...
\Example{
    Show that the function $y = (2x^2+2e^{3x}+3)e^{-2x}$ satisfies the DE \[ \dv{y}{x} + 2y = 6e^x + 4xe^{-2x} \]
}{
    \begin{align*}
        y &= ( 2x^2 + 2e^{3x} + 3 ) e^{-2x} \\
...
\Example{
    Find the solution of the DE $\dv{y}{x} = 2x$ such that $\forall x\in I, f'(x) = 2x$ and  $f(1)=4$
}{
    \begin{align*}
        \dv{y}{x} &= 2x \\
        \int {\dv{y}{x}} \dd{x} &= \int {2x} \dd{x} \\
        y &= x^2 + c
    \end{align*}
...
\Example{$\dv{y}{x} = -\dfrac{x}{y}$, $y(3)=4$}{
    \begin{align*}
        x + y \dv{y}{x} &= 0 \\
        \int{x} \dd{x} + \int{y \dv{y}{x}} \dd{x} &= 0 \\
...
\begin{align*}
        &\forall x\in \mathbb{R}, \quad f_1(x) = 0 \\
        &f_2(x) = (\frac{2}{3}x)^{3/2}, \quad x \ge 0; \quad f_2(x) = 0, \quad x \le 0
\end{align*} are both solutions of this initial-value problem. In fact, this problem has infinitely many solutions. Hence, we can state that the initial-value problem need not have a \textit{unique} solution. In order to ensure uniqeness, some additional requiremnt must certainly be imposed.

\Theorem{Basic Existence and Uniqueness Theorem}{
    \\\textbf{Hypothesis: }
    Consider the differential equation
    \begin{equation} \label{eq:first-order-de}
        \dv{y}{x} = f(x,y)
    \end{equation}
    where
    \begin{itemize}
    ...
    \textbf{Conclusion: }
    There exists a unique solution $\phi$ of the differential equation \eqref{eq:first-order-de}, defined on some interval $|x-x_0| \le h$, where $h$ is sufficiently small, that satisfies the condition
    \[
        \phi(x_0) = y_0
    \]
}
...
\Example{Show that
    ...
    Now,
    for $y = 2e^{2x} + 4e^{-3x}$,
    \[ y_1 = 4e^{2x} - 12e^{-3x}, \quad y_2 = 8e^{2x} + 36e^{-3x} \]
    \begin{align*}
...
\Example{Given that every solution of \[
        x^3 \dv[3]{y}{x} - 3x^2 \dv[2]{y}{x} + 6x \dv{y}{x} - 6y = 0
    \] may be written in the form $y = c_1x + c_2x^2 + c_3x^3$ for some choice of the arbitrary constants $c_1$, $c_2$, and $c_3$, solve the initial-value problem consisting of the above DE plus the three conditions: $y(2)=0$, $y'(2)=2$, $y''(2)=6$
}{
    \[ y = c_1x + c_2x^2 + c_3x^3 \]
    \begin{equation} \label{eq:ex-ivp-1}
        y(2) = 0 \text{ or, } 8c_3 + 4c_2 + 2c_1 = 0
    \end{equation}
    \[ y' = c_1 + 2c_2x + 3c_3x^2 \]
    \begin{equation} \label{eq:ex-ivp-2}
        y'(2) = 2 \text{ or, } 12c_3 + 4c_2 + c_1 = 2
    \end{equation}
    \[ y'' = 0 + 2c_2 + 6c_3x \]
    \begin{equation} \label{eq:ex-ivp-3}
        y''(2) = 6 \text{ or, } 12c_3 + 2c_2 + 0c_1 = 6
    \end{equation}
    Solving \eqref{eq:ex-ivp-1}, \eqref{eq:ex-ivp-2}, and \eqref{eq:ex-ivp-3} we get,

...
\Definition{Linear Ordinary Differential Equation of Order $n$}{
...
\Theorem{Basic Existence Theorem}{
\\\textbf{Hypothesis:}
    \begin{enumerate}
...
\Example{
    ...
    \begin{enumerate}
        \item $\displaystyle y(4)=3$
        \item $\displaystyle y'(4)=5$
        \item $\displaystyle y''(4)=-\frac{7}{2}$
    \end{enumerate}}{
\vspace{-10pt}
...
\Theorem{Basic Theorem on Linear Homogeneous Differential Equations}{
...
\Theorem{Linearly Independent Solutions of nth-Order Linear Differential Equation}{
...
We have observed that $\sin x$ and $\cos x$ are solutions of
    \begin{equation}
        ...
        y'' + y = 0
    \end{equation}
    for all $x \in (-\infty, \infty)$. Further, we can show that these two solutions are linearly independent. Suppose $f$ is any solution of \eqref{eq:sin-cos-de}. Then by Theorem 3.1.4 $f$ can be expressed as a certain linear combination $c_1\sin x + c_2\cos x$ of the two linearly independent solutions  $\sin x$ and $\cos x$ by proper choice of $c_1$ and $c_2$. That is, there exist two particular constants $c_1$ and $c_2$ such that
    \begin{equation}
        f(x) = c_1\sin x + c_2\cos x
    \end{equation}
    for all $x \in (-\infty,\infty)$. For example, it can be easily verified that $f(x) = \sin(x+\pi/6)$ is a solution of the equation \eqref{eq:sin-cos-de}. Since \[
...
\begin{equation}
    \label{eq:second-order-homog}
    \tag{3.1.5}
    a_0(x)y'' + a_1(x)y' + a_2(x)y = 0
\end{equation}
Let's make the transformation
\begin{equation}
    \label{eq:reduction-transform}
    \tag{3.1.6}
    y = f(x)v
\end{equation}
, where $f$ is the known solution of \eqref{eq:second-order-homog} and $v$ is a function of $x$ that will be determined. Then, differentiating, we obtain
\begin{equation}
    \label{eq:reduction-y-prime}
    \tag{3.1.7}
    y' = f(x)v' + f'(x)v,
\end{equation}
\begin{equation}
    \label{eq:reduction-y-double-prime}
    \tag{3.1.8}
    y'' = f(x)v'' + 2f'(x)v' + f''(x)v.
\end{equation}

Substituting \eqref{eq:reduction-transform}, \eqref{eq:reduction-y-prime}, and \eqref{eq:reduction-y-double-prime} into \eqref{eq:second-order-homog}, we obtain \[
...
\]
Since $f$ is a solution of \eqref{eq:second-order-homog}, the coefficient of $v$ is zero, and so the last equation reduces to \[
    a_0(x)f(x)v'' + [ 2a_0(x)f'(x) + a_1(x)f(x) ]v' = 0
\]
Letting $w = v'$, this becomes
\begin{equation}
    \label{eq:reduction-w}
    \tag{3.1.9}
...
\]
This is the general solution of Equation \eqref{eq:reduction-w}; choosing the particular solution for which $c=1$, recalling that $dv/dw = w$, and integrating again, we now obtain \[
    v = \int{ \frac{ \exp\left[ -\int{ \frac{a_1(x)}{a_0(x)} } \: d{x} \right] }{[ f(x) ]^2} } \: d{x} 
\]
Finally, from \eqref{eq:reduction-transform}, we obtain \[
    y = f(x) \int{ \frac{ \exp\left[ -\int{ \frac{a_1(x)}{a_0(x)} } \: d{x} \right] }{[ f(x) ]^2} } \: d{x} 
\]
If we denote the right side of the function by $g$, which is a solution of \eqref{eq:second-order-homog}, we can write the general solution of \eqref{eq:second-order-homog} as the following linear combination \[
    c_1f + c_2g
...
\label{th3.1.8}
    ...
    \begin{equation}
        ...
        a_0(x)y'' + a_1(x)y' + a_2(x)y = 0
    \end{equation}
    \textbf{Conclusion 1:} The transformation $y = f(x)v$ reduces \eqref{eq:second-order-general} to the first-order linear homogeneous DE \[
        a_0(x)f(x)\frac{dw}{dx} + [ 2a_0(x)f'(x) + a_1(x)f(x) ]w = 0
    ...
    \] of equation \eqref{eq:second-order-general} gives rise to the function $v$, where \[
    v(x) = \int{ \frac{\exp\left[ -\int{\frac{a_1(x)}{a_0(x)}} \: d{x} \right]}{[f(x)]^2} } \: d{x}
    \] The function $g(x)$ defined by $g(x) = f(x)v(x)$ is then a solution of the second-order equation \eqref{eq:second-order-general}. \\
    \textbf{Conclusion 3:} The original known solution $f$ and the new solution $g$ are linearly independent solutions of \eqref{eq:second-order-general}, and hence the general solution of \eqref{eq:second-order-general} is \[
        c_1f + c_2g
    \] where $c_1$ and $c_2$ are arbitrary constants.
}
...
\end{align*}
Substituting into \eqref{eq:double-root-example} we obtain
\begin{align*}
...
\]
By Theorem 3.1.7 we know that for any choice of $c_0$, a solution of the given second-order equation \eqref{eq:double-root-example} is \[
...
Thus the general solution of \eqref{eq:double-root-example} may be written as \[
    y = c_1e^{3x} + c_2xe^{3x} \]
...
Let's assume
\begin{equation}
    \label{eq:undetermined-assumption1}
    y_p = Ae^{4x}
\end{equation}
Here $A$ is the undetermined coefficient to be determined. \[
    y_p' = 4Ae^{4x}, y_p'' = 16Ae^{4x}
\] Substituting into \eqref{eq:undetermined-ex1}, we obtain
\begin{align*}
...
\begin{equation}
    \label{eq:undetermined-ex2}
    \tag{3.3.4}
    y'' - 2y' - 3y = 2e^{3x}
\end{equation}
Here if we assume
\begin{equation}
    \label{eq:undetermined-assumption2}
    \tag{3.3.5}
    y_p = Ae^{3x}
\end{equation}
Then \[
...
\]
Substituting into \eqref{eq:undetermined-ex2}, we obtain
\begin{align*}
    9Ae^{3x} - 2(3Ae^{3x}) - 3(Ae^{3x}) &= 2e^{3x} \\
    0 \cdot Ae^{3x} &= 2e^{3x} \\
    0 &= 2e^{3x}
\end{align*}
which is not possible, and hence there is no particular integral of the form \eqref{eq:undetermined-assumption2}.

\rule{4in}{0.25pt}

Let's examine the homogeneous equation 
\begin{equation}
    \label{eq:undetermined-homog}
    \tag{3.3.6}
    y'' - 2y' - 3y = 0
\end{equation}
Auxiliary equation:
...
The failure to obtain a solution of the form \eqref{eq:undetermined-assumption2} is due to the fact that  $e^{3x}$ is a solution of the corresponding homogeneous equation of \eqref{eq:undetermined-ex2}. Since $e^{3x}$ satisfies the homogeneous equation, it reduces the left member of the equation to 0, not $2e^{3x}$.
...
\begin{equation}\tag{3.3.6}
    y'' - 2y' - 3y = 0
\end{equation}
Equation \eqref{eq:undetermined-homog} has solutions $e^{3x}$ and $e^{-1}$ \[
    \therefore y_c = c_1e^{3x} + e^{-1}
\]
The right member of \eqref{eq:undetermined-ex1}, $2e^{4x}$, is not a solution of \eqref{eq:undetermined-homog}
\begin{equation}
...
\]
The right member of \eqref{eq:undetermined-ex2}, $e^{2e^{3x}}$ is a solution of \eqref{eq:undetermined-homog}
\begin{equation}
...
\vspace{-20pt}
    ...
    \begin{align*}
        &\left[ Ax^4e^{x} + (8A+B)x^3e^{x} + (12A+6B+C)x^2e^{x} + (6B+4C)xe^{x} + 2Ce^{x} \right] \\
        &- 2 \left[ Ax^4e^{x} + (4A+B)x^3e^{x} + (3B+C)x^2e^{x} + 2Cxe^{x} \right] \\
        &+ \left[ Ax^4e^{x} + Bx^3e^{x} + Cx^2e^{x} \right] = x^2e^{x}
    \end{align*}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\vspace{20pt}\rule{3in}{1pt}
\subsection{Variationg of Parameters}
...
The method of undetermined coefficients is quite straightforward, but its application is also restricted. Thus we seek a general method that applies in all cases. Let a nonhomogeneous linear differential equation be
\begin{equation}
    \label{eq:vop-nonhomog}
    \tag{3.4.1}
...
\begin{equation}
    ...
    a_0(x)y'' + a_1(x)y' + a_2(x)y = 0
\end{equation}
Suppose the complimentary functions \[
    y_c = c_1y_1(x) + c_2y_2(x)
\]
Now, if we replace $c_1$ and $c_2$ by functions $v_1$ and $v_2$ respectively, which will be determined such that the function
\begin{equation}
    v_1(x)y_1(x) + v_2(x)y_2(x)
\end{equation}
will be a particular integral of \eqref{eq:vop-nonhomog}. We can write \[
    y_p(x) = v_1(x)y_1(x) + v_2(x)y_2(x)
\] \[
    y_p'(x) = v_1(x)y_1'(x) + v_2(x)y_2'(x) + v_1'(x)y_1(x) + v_2'(x)y_2(x)
\] Assuming \[
...
\] \[
    y_p''(x) = v_1(x)y_1''(x) + v_2(x)y_2''(x) + v_1'(x)y_1'(x) + v_2'(x)y_2'(x)
\]
Substituting into \eqref{eq:vop-nonhomog},
\begin{equation*}
    \begin{split}
        a_0(x)  &[ v_1(x)y_1''(x) + v_2(x)y_2''(x) + v_1'(x)y_1'(x) + v_2'(x)y_2'(x) ]\\
                & + a_1(x)[ v_1(x)y_1'(x) + v_2(x)y_2'(x) ]\\
                & + a_2(x)[ v_1(x)y_1(x) + v_2(x)y_2(x) ] = F(x)
    \end{split}
\end{equation*} or,
\begin{equation*}
    \begin{split}
        v_1(x)  &[a_0(x)y_1'' + a_1(x)y_1' + a_2(x)y_1]\\
                & + v_1(x)[a_0(x)y_2'' + a_1(x)y_2' + a_2(x)y_2]\\
                & + a_0(x)[ v_1'(x)y_1'(x) + v_2'(x)y_2'(x) ] = F(x)
    \end{split}
\end{equation*}
Since $y_1$ and $y_2$ are solutions of \eqref{eq:vop-homog}, we get \[
     v_1(x)y_1(x) + v_2(x)y_2(x) = \frac{F(x)}{a_0(x)}
\]
Now, $v_1$ and $v_2$ have to be chosen such that the following system of equation is satisfied:
\begin{align}
    \label{eq:vop-system}
    \tag{3.4.4}
    y_1(x)v_1'(x) + y_2(x)v_2'(x) &= 0 \\
    y_1'(x)v_1'(x) + y_2'(x)v_2'(x) &= \frac{F(x)}{a_0(x)} \notag
\end{align}
Now,
\[
    \begin{bmatrix}
...
\] Thus we obtain $v_1$ and $v_2$ defined by
\begin{equation}
    \boxed{ 
    \begin{split}
        v_1(x) &= - \int^{x} { \frac{F(t)y_2(t)}{a_0(t)W[y_1(t),y_2(t)]} } \: d{t} \\
        v_2(x) &= \int^{x} { \frac{F(t)y_1(t)}{a_0(t)W[y_1(t),y_2(t)]} } \: d{t}
    \end{split}
    }
\end{equation}
Therefore, a particular integral $y_p$ of \eqref{eq:vop-nonhomog} is defined by \[
    y_p = v_1(x)y_1(x) + v_2(x)y_2(x)
\] where $v_1$ and $v_2$ are defined by \eqref{eq:vop-system}.

...
\Example{Find the tangent plane and normal line to $9yz - \sqrt{x^2-8z} = xy^2 - 26$ at $(3,1,-2)$}{
    ...
    \[ \frac{x-3}{ \frac{8}{5} } = \frac{y-1}{24} = \frac{z+2}{ - \frac{49}{5} } \]
...
\Theorem{Extreme Value Theorem}{
    If $f(x,y)$ is continuous on a closed and bounded region $D$ in $\R^2$, then there are two points $(x_1,y_1)$ and $(x_2,y_2)$ in region $D$ such that $f(x_1,y_1)$ is the absolute maximum and $f(x_2,y_2)$ is the absolute minimum of $f(x,y)$ in $D$.
}
...
\Example{Find the absolute minimum and absolute maximum of $f(x,y)=18x^2+4y^2-y^2x-2$ on the triangle with vertices $(-1,-1)$, $(5,-1)$, and $(5,17)$.}{
    ...
    \begin{minipage}{0.34\textwidth}
        ...
        \begin{tikzpicture}[scale=0.22]
            % Grid lines
            \draw[step=1cm,gray!30,very thin] (-2,-2) grid (6,18);
            % Axes
            \draw[very thick,->] (-2,0) -- (6,0) node[right] {$x$};
            \draw[very thick,->] (0,-2) -- (0,18) node[above] {$y$};
            % Triangle vertices
            \coordinate (A) at (-1,-1);
            \coordinate (B) at (5,-1);
            \coordinate (C) at (5,17);
            % Triangle edges
            \draw[thick] (A) -- (B) -- (C) -- cycle;
            % Labels for vertices
            \node[below left] at (A) {(-1,-1)};
            \node[below right] at (B) {(5,-1)};
            \node[above right] at (C) {(5,17)};
        \end{tikzpicture} \vspace{-0.2cm}
    ...
    \end{align*}
    For segment 2: \\ 
    Let $h(y)=f(5,y) = 450 + 4y^2 - 5y^2 - 2 = 448 - y^2$ \\ 
    ...
    Hence, the absolute extrema are given by
    \begin{align*}
        &\text{Absolute Maximum: } (5,-1) \text{ with value } 447, \\ 

...
\Definition{Smooth Curve}{
    ...
    \] \[ 
        \text{or } \vec{r}(t) = x(t) \hat{i} + x(t) \hat{j} \quad a \leq t \leq b
    \]
...
\Example{Evaluate $\int \limits_{C} xyz \dd{s}$ where $C$ is the curve given by $x = t$, $y = t^2$, $z = t^3$ for $0 \le t \le 1$.}{
    We have $x(t) = t$, $y(t) = t^2$, and $z(t) = t^3$.
    
    First, compute $\dd{s}$:
    \[
        \dv{x}{t} = 1 \quad \dv{y}{t} = 2t \quad \dv{z}{t} = 3t^2
    \]
    \[ \therefore \dd{s} = \sqrt{1 + 4t^2 + 9t^4} \dd{t} \]
    
    Now evaluate the integral: \[
        \int \limits_{C} xyz \dd{s} = \int_0^1 (t)(t^2)(t^3) \sqrt{1 + 4t^2 + 9t^4} \dd{t} = \int_0^1 t^6 \sqrt{1 + 4t^2 + 9t^4} \dd{t}
    \]
    
    Notice that $1 + 4t^2 + 9t^4 = (1 + 3t^2)^2$, so:
    \begin{align*}
        \int \limits_{C} xyz \dd{s} &= \int_0^1 t^6 (1 + 3t^2) \dd{t} \\
        &= \int_0^1 (t^6 + 3t^8) \dd{t} \\
        &= \frac{t^7}{7}\Biggr|_{0}^{1} + \frac{3t^9}{9}\Biggr|_{0}^{1} \\
        &= \frac{1}{7} + \frac{1}{3} = \frac{10}{21}
    \end{align*}
}
...
\Example{Let $\vec{F}(x,y) = x^2\,\vec{i} + (2x+y)\,\vec{j}$. Evaluate $\vec{F}(1,2)$.}{
    We simply substitute the point into the component functions:
    \begin{align*}
        \vec{F}(1,2) &= (1)^2\,\vec{i} + (2(1)+2)\,\vec{j} \\
        &= \vec{i} + 4\,\vec{j}
    \end{align*}
}
...
\Theorem{Fundamental Theorem for Line Integrals}{
    Let $C$ be a smooth curve given by $\vec{r}(t), a<=t<=b$. Also, let $f$ be a differentiable scalar function whose gradient vector field $\nabla f$ is continuous on $C$. Then:
    \[ 
...
\Note{
    \begin{enumerate}
        ...
        \item If $\displaystyle \int \limits_{C} \vec{F} \cdot \dd{\vec{r}}$ is independent of path, then $\displaystyle \int \limits_{C} \vec{F} \cdot \dd{r} = 0$ for every closed path $C$.
...
\begin{align*}
    \iint \limits_{D} (Q_x-P_y) \dd{A} &= \iint \limits_{D_1} (Q_x-Py) \dd{A} + \iint \limits_{D_2} (Q_x-P_y) \dd{A} \\
